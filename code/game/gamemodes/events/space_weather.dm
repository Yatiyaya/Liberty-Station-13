...
/datum/event/bluespace_storm/announce()
	command_announcement.Announce("The scanners have detected a bluespace storm near the planet. Bluespace distortions are likely to happen while it lasts.", "Bluespace Storm")
...
/datum/event/bluespace_storm/tick()
	if(!(activeFor % 120)) //Every 120 ticks
...
/datum/event/ion_blizzard/setup()
	endWhen = rand(100, 200)

/datum/event/ion_blizzard/start()
	SSevent.change_parallax("ion_blizzard")

/datum/event/ion_blizzard/announce()
	command_announcement.Announce("A severe solar flare has been detected near the colony. Lighting subsystems are currently overloaded and may not work properly.", "Ion Blizzard")
...
/datum/event/ion_blizzard/tick() //get random ship area and do things to all light in there
	if(!(activeFor % 80))	// Every 80th tick
		if(activeFor % 2)
...
/datum/event/photon_vortex/announce()
	command_announcement.Announce("A photon vortex anomaly has been detected near the planet. All photon-emitting machinery gives much less light.", "Photon Vortex Anomaly")
/datum/event/photon_vortex/end()
	command_announcement.Announce("The photon vortex anomaly has moved away from the planet.", "Photon Vortex Anomaly")
	SSevent.change_parallax(GLOB.random_parallax)
	for(var/obj/item/device/lighting/L in world)
...
/datum/event/micro_debris/announce()
	command_announcement.Announce("The planet is now passing through a micro debris field.", "Micro Debris Field Alert")

/datum/event/micro_debris/end()
	command_announcement.Announce("The planet has now passed through the micro debris field.", "Micro Debris Field Notice")
	SSevent.change_parallax(GLOB.random_parallax)
...
/datum/event/graveyard/announce()
	command_announcement.Announce("Drifting wrecks of a space station have been detected near the planet. Telecommunication systems are not responsible for any strain on the crew's psychological wellbeing.", "Space Graveyard")
...
/datum/event/graveyard/end()
	command_announcement.Announce("The station wrecks have moved away from the planet.", "Space Graveyard")
...
/datum/event/nebula/announce()
	command_announcement.Announce("Uncharacteristically high concentrations of dark matter from a nearby nebula currently passing through the local star sector. Crew might experience certain issues with their mental wellbeing.", "Dark Matter Nebula")
