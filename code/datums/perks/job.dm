...
/datum/perk/job/active_sanityboost/assign(mob/living/carbon/human/H)
	if(..())
		holder.sanity.sanity_passive_gain_multiplier *= 1.5

/datum/perk/job/active_sanityboost/remove()
	if(holder)
		holder.sanity.sanity_passive_gain_multiplier /= 1.5
	..()
...
/datum/perk/job/neat/assign(mob/living/carbon/human/H)
	if(..())
		holder.sanity.view_damage_threshold += 20

/datum/perk/job/neat/remove()
	if(holder)
		holder.sanity.view_damage_threshold -= 20
	..()
...
/datum/perk/job/chemist/chemist/remove()
	if(holder)
		holder.metabolism_effects.nsa_mult -= 0.25
		holder.metabolism_effects.calculate_nsa()
	..()
