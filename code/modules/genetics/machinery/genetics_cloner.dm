...
/obj/machinery/genetics/cloner/attackby(obj/item/I, mob/user)
	if(!user.stats?.getPerk(PERK_SCIENCE) && !usr.stat_check(STAT_COG, 90) &&!user.stats?.getPerk(PERK_CHEMIST) && !usr.stat_check(STAT_BIO, 180))
...
/obj/machinery/computer/genetics/clone_console/attack_hand(mob/user)
	if(!user.stats?.getPerk(PERK_SCIENCE) && !usr.stat_check(STAT_COG, 75) &&!user.stats?.getPerk(PERK_CHEMIST) && !usr.stat_check(STAT_BIO, 150))
