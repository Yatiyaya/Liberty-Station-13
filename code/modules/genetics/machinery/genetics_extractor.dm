...
/obj/machinery/genetics/pulper/attackby(obj/item/I, mob/user)
	if(!user.stats?.getPerk(PERK_SI_SCI) && !usr.stat_check(STAT_COG, 35) && !user.stats?.getPerk(PERK_CHEMIST) && !usr.stat_check(STAT_BIO, 70)) //So someone that has basic chems or level up can be an assent
