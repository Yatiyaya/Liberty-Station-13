...
/datum/lecture/proc/perform(mob/living/carbon/human/H, obj/item/implant/core_implant/C, targets)
	return TRUE

//ritual will be proceed only if this returns true
/datum/lecture/proc/pre_check(mob/living/carbon/human/H, obj/item/implant/core_implant/C, targets)
...
/datum/lecture/proc/failed(mob/living/carbon/human/H, obj/item/implant/core_implant/C, targets, on_chance = FALSE)
	return

/datum/lecture/proc/activate(mob/living/carbon/human/H, obj/item/implant/core_implant/C, var/list/targets, var/force = FALSE)
...
/datum/lecture/proc/fail(var/message, mob/living/carbon/human/H, obj/item/implant/core_implant/C, targets)
	if(!message)
		message = fail_message
	to_chat(H, SPAN_DANGER("[message]"))
	failed(H, C, targets)

/datum/lecture/proc/check_success(obj/item/implant/core_implant/C)
	return prob(chance * C.success_modifier)

/datum/lecture/proc/is_allowed(obj/item/implant/core_implant/C)
	return TRUE

//returns phrase to say, may require to specify target
/datum/lecture/proc/get_say_phrase()
	return phrase

//returns phrase to display in bible
/datum/lecture/proc/get_display_phrase()
	return phrase

//returns true, if text is phrase of this ritual
/datum/lecture/proc/compare(var/text)
	return phrase && phrase != "" && text == phrase

//returns list of targets, specified in text
/datum/lecture/proc/get_targets(var/text)
	return list()


//COOLDOWN STUFF
//sets global cooldown for ritual's cooldown category
/datum/lecture/proc/set_global_cooldown()
	if(src.cooldown)
		GLOB.global_lecture_cooldowns[src.cooldown_category] = TRUE
...
/datum/lecture/proc/reset_global_cooldown()
	GLOB.global_lecture_cooldowns[src.cooldown_category] = FALSE

//sets personal cooldown for user of ritual's cooldown category
/datum/lecture/proc/set_personal_cooldown(mob/living/carbon/human/user)
	if(src.cooldown)
...
/datum/lecture/proc/is_on_cooldown(mob/living/carbon/human/user)
	if(GLOB.global_lecture_cooldowns[src.cooldown_category])
		return TRUE
